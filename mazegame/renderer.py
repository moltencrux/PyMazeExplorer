...
EXPLORED_GREY = (0x60, 0x60, 0x60)  # mid grey – distinct from unexplored black
FADE_MOVES = 48  # algorithm steps until a cell's highlight is fully grey
...
class MazeRenderer:
    def __init__(self) -> None:
        ...
        # --- Exploration overlay ---
        # cell -> explore-generation when last marked (fade advances with moves)
        self._explored: Dict[Cell, int] = {}
        self._explore_gen: int = 0
        # Cells still transitioning highlight → grey
    ...
    def set_engine_state(
        ...
        self._bg_scaled = self._exp_scaled = None
    ...
    def mark_explored(self, cell: Cell) -> None:
        """Record visit; fade progress is move-based via _explore_gen."""
        with self._explore_lock:
            self._explore_gen += 1
            self._explored[cell] = self._explore_gen
    ...
    def mark_explored_many(self, cells: List[Cell]) -> None:
        with self._explore_lock:
            for c in cells:
                self._explore_gen += 1
                self._explored[c] = self._explore_gen
                self._fading.add(c)
    ...
    def animate_hop(self, from_cell: Cell, to_cell: Cell) -> None:
        from_px = self._cell_center(from_cell)
        to_px = self._cell_center(to_cell)
        self._anim_kind = "hop"
        self._anim_from = from_px
    ...
    def advance_anim(self, edge_delta: float) -> float:
        ...
        if self._anim_kind in ("slide", "hop"):
    ...
    def _spring_step(
        ...
        # Settle when close enough — kills post-solve micro-oscillation.
        if abs(pos - target) < 0.5 and abs(vel) < 2.0:
            return target, 0.0
    ...
    def draw(
        ...
        # Prefer exact 1:1 when nearly there — avoids shimmer on wall edges.
        if abs(z - 1.0) < 0.02:
            z = 1.0
        # Pixel-snap camera so wall edges don't shimmer under sub-pixel motion
        world_cx = round(world_cx * z) / z
        world_cy = round(world_cy * z) / z
        ...
        # Integer scale size; at z≈1 this is 1:1 nearest-neighbour.
        scaled_w = max(1, int(round(src_w * z)))
        scaled_h = max(1, int(round(src_h * z)))

        # Screen position of world (0,0) — integer so blit lands on pixel grid.
        ox = round(viewport.centerx - world_cx * z)
        oy = round(viewport.centery - world_cy * z)
        dest_x = int(ox + round(src_x * z)) - viewport.x
        dest_y = int(oy + round(src_y * z)) - viewport.y
        ...
        cache_key = (src_x, src_y, src_w, src_h, scaled_w, scaled_h, abs(z - 1.0) < 1e-6)
        need_rescale = cache_key != self._scale_cache_key

        # Nearest-neighbour at exact 1:1 (sharp); bilinear otherwise so hard
        # cyan/black wall edges don't shimmer under sub-pixel camera motion.
        scale_fn = (
        ...
        if need_rescale or self._bg_scaled is None:
            ...
            self._bg_scaled = scale_fn(bg_src, (scaled_w, scaled_h))
        ...
        if self.explore_surface is not None:
            if need_rescale or fading_active or self._exp_scaled is None:
                ...
                self._exp_scaled = scale_fn(exp_src, (scaled_w, scaled_h))
            view_surf.blit(self._exp_scaled, (dest_x, dest_y))
        # Path trail intentionally disabled (exploration overlay is enough).
    ...
    def _update_exploration_fade(self) -> bool:
        """Repaint fading cells from move-age (no progress if solver is idle)."""
        with self._explore_lock:
            if not self._fading:
                return False
            fading = list(self._fading)
            explored = dict(self._explored)
            gen = self._explore_gen

        done: List[Cell] = []
        any_painted = False
        for cell in fading:
            marked_at = explored.get(cell)
            if marked_at is None:
                done.append(cell)
                continue
            age_moves = gen - marked_at
            if age_moves >= FADE_MOVES:
                self._paint_explored_cell(cell, EXPLORED_GREY)
                done.append(cell)
                any_painted = True
            else:
                raw = 1.0 - age_moves / float(FADE_MOVES)
                intensity = raw * raw
